\capitulo{6}{Selección de modelo y proceso de implementación}

Para seleccionar el modelo y forma de adaptación, se han de realizar diferentes pruebas, así como ver que cantidad de datos tenemos para realizar la adaptación.

\section{Pruebas realizadas sobre los modelos}
...
El modelo se adaptará siguiendo el enfoque de Lora (Low Rank Adaptation) debido a que no se dispone de suficientes ejemplos de código Java.
...
\subsection{Resultados que surgieron con Code-Llama tras el entrenamiento con Lora}

Tras diversas pruebas y tareas de optimización sobre el dataset, se obtuvieron los siguientes resultados:
...
\section{Reemplazo del modelo}

Se decidió, ya que la implementación del código era muy similar, realizar el entrenamiento con el modelo superior Llama 3, ya que este en las pruebas preeliminares de selección de modelo había obtenido unos resultados similares en los \textit{prompts} y su compatibilidad multilenguaje es incluso mejor que la de codeLlama. El único problema es que este modelo no esta especializado en código, pero al tratarse de un modelo pensado para que se use en asignaturas de iniciación a la programación es mas que suficiente para el objetivo del proyecto.

Sorprendentemente la tasa de aprendizaje con el mismo conjunto de datos era muy superior ademas de que los resultados de los \textit{prompts} tanto en lenguaje natural como con código resultaron ser muy superiores a los ofrecidos por codeLlama.
